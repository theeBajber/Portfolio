...
export default function Taskbar({ className, onCalcClick, onContactClick }) {
  useEffect(() =>{
    ...
    buttons.forEach((btn, i) => {
      btn.addEventListener("mouseenter", () => {
        for (let j = -2; j <= 2; j++) {
          ...
          buttons[index].style.transform = `translateY(${offset}px) scaleY(${scale})`;
        }
      })
  ...
  return (
    <div className={`backdrop-blur-2xl rounded-3xl bg-white/20 flex justify-evenly items-center px-2 ${className} min-w-87 taskbar`}>
      <button>
